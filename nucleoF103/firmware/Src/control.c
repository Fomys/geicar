...
void car_control(int left_rear_speed, int right_rear_speed, int steering_speed, int current_left, int current_right){
	...
	if (left_rear_speed == DISABLED && right_rear_speed == DISABLED && steering_speed == DISABLED){
		...
		//Propulsion
		if (left_rear_speed - current_left > TOLERANCE)
		{
			speed_left = PI_speed_left(left_rear_speed, current_left);
		}
		if (right_rear_speed- current_right > TOLERANCE)
		{
		...
		//Steering
		if (steering_speed - steering_get_angle() > TOLERANCE)
		{
			angle = PI_angle(steering_speed);
			steering_set_speed(GPIO_PIN_SET, angle);
		}
	}
}

/*
 * PI Controller Computespeed left
 * Input: speed in rpm [0 ; 100]
 * Output: PWM [0 ; 100]
 */
int PI_speed_left(int left_rear_speed, int currentLeftRearSpeed)
{
	...
	if ( left_rear_speed - OFFSET >= 0) //allez avant
	{

		//Computation of the error for Kp
		speedErrorLeft = left_rear_speed - currentLeftRearSpeed - OFFSET;
	...
	else if (left_rear_speed - OFFSET < 0) //ALLEZ ARRIÈRE
	{
		//Computation of the error for Kp
		speedErrorLeft = left_rear_speed + currentLeftRearSpeed - OFFSET;
...
int PI_speed_right(int right_rear_speed, int currentRightRearSpeed)
{
	...
	//Computation of the error for Kp
	speedErrorRight = right_rear_speed - currentRightRearSpeed - OFFSET;
